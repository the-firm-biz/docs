...
<Warning>
  AutoBoy always aims to give priority to 'good' holders. Order prioritisation will adapt and is subject to change.
</Warning>
...
2. **Market cap descending** (tiebreaker when two orders share the same spend amount): i.e. execution is attempted for orders willing to buy at the highest market cap first (where ‘No limit’ is the highest).
